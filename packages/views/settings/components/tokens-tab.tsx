...
export function TokensTab() {
  ...
  const loadTokens = useCallback(async () => {
    try {
      ...
      toast.error(e instanceof Error ? e.message : t.settings.failedLoadTokens);
    } finally {
  ...
  const handleCreateToken = async () => {
    ...
    try {
      ...
      toast.error(e instanceof Error ? e.message : t.settings.failedCreateToken);
    } finally {
  ...
  const handleRevokeToken = async (id: string) => {
    ...
    try {
      ...
      toast.error(e instanceof Error ? e.message : t.settings.failedRevoke);
    } finally {
  ...
  return (
    <div className="space-y-8">
            ...
            <div className="grid gap-3 sm:grid-cols-[1fr_120px_auto]">
              ...
              <Select value={tokenExpiry} onValueChange={(v) => { if (v) setTokenExpiry(v); }}>
                ...
                <SelectContent>
                  ...
                  <SelectItem value="365">1 {t.settings.year}</SelectItem>
              ...
              <Button onClick={handleCreateToken} disabled={tokenCreating || !tokenName.trim()}>
                {tokenCreating ? t.settings.updating : t.settings.create}
              </Button>
            </div>
          </CardContent>
        </Card>
                ...
                <CardContent className="flex items-center gap-3">
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="text-xs text-muted-foreground">
                      {token.token_prefix}... · {t.settings.createdOn.replace("{date}", new Date(token.created_at).toLocaleDateString())} · {token.last_used_at ? t.settings.lastUsed.replace("{date}", new Date(token.last_used_at).toLocaleDateString()) : t.settings.neverUsed}
                      {token.expires_at && ` · ${t.settings.expires.replace("{date}", new Date(token.expires_at).toLocaleDateString())}`}
                    </div>
                  </div>
                  <Tooltip>
                    <TooltipTrigger
      ...
      <AlertDialog open={!!revokeConfirmId} onOpenChange={(v) => { if (!v) setRevokeConfirmId(null); }}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>{t.settings.revokeTitle}</AlertDialogTitle>
            <AlertDialogDescription>

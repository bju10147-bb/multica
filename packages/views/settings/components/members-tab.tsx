...
function MemberRow({
  ...
  const { t } = useLocale();
  const roleConfig = {
    owner: { label: t.settings.roles.owner, icon: Crown, description: t.settings.roleDescriptions.owner },
    admin: { label: t.settings.roles.admin, icon: Shield, description: t.settings.roleDescriptions.admin },
    member: { label: t.settings.roles.member, icon: User, description: t.settings.roleDescriptions.member },
  };
...
export function MembersTab() {
  ...
  const handleInviteMember = async () => {
    ...
    try {
      ...
      toast.error(e instanceof Error ? e.message : t.settings.failedSendInvitation);
    } finally {
    ...
    setConfirmAction({
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          toast.error(e instanceof Error ? e.message : t.settings.failedRevokeInvitation);
        } finally {
  ...
  const handleRoleChange = async (memberId: string, role: MemberRole) => {
    ...
    try {
      ...
      toast.error(e instanceof Error ? e.message : t.settings.failedUpdateMember);
    } finally {
  ...
  const handleRemoveMember = (member: MemberWithUser) => {
    if (!workspace) return;
    setConfirmAction({
      title: t.settings.removeMemberTitle.replace("{name}", member.name),
      ...
      onConfirm: async () => {
        ...
        try {
          ...
          toast.error(e instanceof Error ? e.message : t.settings.failedRemoveMember);
        } finally {
          setMemberActionId(null);
        }
      },
    ...
    <div className="space-y-8">
      <section className="space-y-4">
        <div className="flex items-center gap-2">
          ...
          <h2 className="text-sm font-semibold">{t.settings.membersCount.replace("{n}", members.length.toString())}</h2>
        </div>
            ...
            <CardContent className="space-y-3">
              ...
              <div className="grid gap-3 sm:grid-cols-[1fr_120px_auto]">
                ...
                <Select value={inviteRole} onValueChange={(value) => setInviteRole(value as MemberRole)}>
                  ...
                  <SelectContent>
                    <SelectItem value="member">{t.settings.roles.member}</SelectItem>
      ...
      {invitations.length > 0 && (
        <section className="space-y-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-semibold">{t.settings.pendingInvitations.replace("{n}", invitations.length.toString())}</h2>
          </div>
          <div className="overflow-hidden rounded-xl ring-1 ring-foreground/10">

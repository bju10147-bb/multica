...
export function AgentDetail({
  ...
  const detailTabs: { id: DetailTab; label: string; icon: typeof FileText }[] = [
    { id: "instructions", label: t.agents.tabs.instructions, icon: FileText },
    { id: "skills", label: t.agents.tabs.skills, icon: BookOpenText },
    { id: "tasks", label: t.agents.tabs.tasks, icon: ListTodo },
    { id: "env", label: t.agents.tabs.env, icon: KeyRound },
    { id: "custom_args", label: t.agents.tabs.customArgs, icon: Terminal },
    { id: "settings", label: t.agents.tabs.settings, icon: Settings },
  ];
          ...
          <div className="flex items-center gap-2">
            ...
            {isArchived ? (
              ...
              <span className={`flex items-center gap-1.5 text-xs ${st.color}`}>
                ...
                {t.agents.status[agent.status] ?? st.label}
              </span>
            )}
          ...
          <DialogContent className="max-w-sm" showCloseButton={false}>
            <div className="flex items-center gap-3">
              ...
              <DialogHeader className="flex-1 gap-1">
                <DialogTitle className="text-sm font-semibold">{t.agents.archiveTitle}</DialogTitle>
                <DialogDescription className="text-xs">
                  {t.agents.archiveDesc.replace("{name}", agent.name)}
                </DialogDescription>
              </DialogHeader>
            </div>
            <DialogFooter>

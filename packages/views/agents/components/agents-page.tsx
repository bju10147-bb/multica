...
export function AgentsPage() {
  ...
  const handleUpdate = async (id: string, data: Record<string, unknown>) => {
    try {
      ...
      toast.error(e instanceof Error ? e.message : t.agents.failedToUpdate);
      throw e;
    }
  };
  ...
  const handleArchive = async (id: string) => {
    try {
      ...
      toast.error(e instanceof Error ? e.message : t.agents.failedToArchive);
    }
  };
  ...
  const handleRestore = async (id: string) => {
    try {
      ...
      toast.error(e instanceof Error ? e.message : t.agents.failedToRestore);
    }
  };
          ...
          {filteredAgents.length === 0 ? (
            <div className="flex flex-col items-center justify-center px-4 py-12">
              ...
              <p className="mt-3 text-sm text-muted-foreground">
                {showArchived ? t.agents.empty.noArchived : archivedCount > 0 ? t.agents.empty.noActive : t.agents.empty.noAgents}
              </p>
              {!showArchived && (
                <Button
                  onClick={() => setShowCreate(true)}
      ...
      <ResizablePanel id="detail" minSize="50%">
        ...
        {selected ? (
          ...
          <div className="flex h-full flex-col items-center justify-center text-muted-foreground">
            ...
            <p className="mt-3 text-sm">{t.agents.empty.selectAgent}</p>
            <Button
              onClick={() => setShowCreate(true)}
              size="xs"
              className="mt-3"

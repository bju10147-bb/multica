...
import { InboxDetailLabel, useInboxI18n } from "./inbox-detail-label";
import { useLocale } from "@multica/core";

export function InboxPage() {
  const { t } = useLocale();
  const { typeLabels } = useInboxI18n();
  ...
  const handleSelect = (item: InboxItem) => {
    ...
    if (!item.read) {
      markReadMutation.mutate(item.id, {
        onError: () => toast.error(t.inbox.failedToMarkRead),
      });
    }
  };
  ...
  const handleArchive = (id: string) => {
    ...
    archiveMutation.mutate(id, {
      onError: () => toast.error(t.inbox.failedToArchive),
    });
  };
  ...
  const handleMarkAllRead = () => {
    markAllReadMutation.mutate(undefined, {
      onError: () => toast.error(t.inbox.failedToMarkAllRead),
    });
  };
  ...
  const handleArchiveAll = () => {
    ...
    archiveAllMutation.mutate(undefined, {
      onError: () => toast.error(t.inbox.failedToArchiveAll),
    });
  };
  ...
  const handleArchiveAllRead = () => {
    ...
    archiveAllReadMutation.mutate(undefined, {
      onError: () => toast.error(t.inbox.failedToArchiveRead),
    });
  };
  ...
  const handleArchiveCompleted = () => {
    ...
    archiveCompletedMutation.mutate(undefined, {
      onError: () => toast.error(t.inbox.failedToArchiveCompleted),
    });
  };
  ...
  const detailContent = selected?.issue_id ? (
    ...
    <div className="p-6">
      ...
      <p className="mt-1 text-sm text-muted-foreground">
        {typeLabels[selected.type]} · {timeAgo(selected.created_at, t)}
      </p>
      {selected.body && (
